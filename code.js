...
// Cores
const TICK_LINE_COLOR = { r: 0.710, g: 0.725, b: 0.831 }; // #b5b9d4
const TICK_TEXT_COLOR = { r: 0.392, g: 0.396, b: 0.529 }; // #646587
const WHITE = { r: 1, g: 1, b: 1 };
const DARK_TEXT = { r: 0.157, g: 0.157, b: 0.204 }; // #282834
const CONTAINER_BG = { r: 1, g: 1, b: 1 }; // branco
const EASING_PILL_BG = { r: 0.263, g: 0.271, b: 0.396 }; // #434565
const TRIGGER_BG = { r: 0.945, g: 0.957, b: 1.0 }; // #f1f4ff (atualizado p/ referência)
const TRIGGER_BTN_BG = { r: 0.157, g: 0.157, b: 0.204 }; // #282834
const DELAY_LINE_COLOR = { r: 0.557, g: 0.569, b: 0.659 }; // #8e91a8
const COMP_CONTAINER_BG = { r: 189 / 255, g: 194 / 255, b: 218 / 255 }; // #BDC2DA (20% opacidade)
...
const CARDS_BG = { r: 0.824, g: 0.862, b: 1.0 }; // #D2DBFF (30% opacidade)
...
function mapPropertyToVariant(propType) {
...
function getFromToPropertyKeys(variant) {
    switch (variant) {
...
function easingToVariant(easing) {
    const e = (easing || '').toLowerCase();
    if (e.includes('enter'))
        return 'Enter';
    if (e.includes('exit'))
        return 'Exit';
    if (e.includes('linear'))
        return 'Linear';
    if (e.includes('onscreen') || e.includes('on screen') || e.includes('on_screen'))
        return 'On screen';
    if (e.includes('instante') || e.includes('instant'))
        return 'Instante';
    return 'On screen';
}
// ============================================================
...
function createLabel(text, fontSize, color, fontStyle = "Medium", fontFamily = "Inter") {
...
function getFormattedFromValue(prop) {
    switch (prop.type) {
        case "position": {
...
function getFormattedToValue(prop) {
    switch (prop.type) {
        case "position": {
            const axis = prop.axis || "Y";
            const val = prop.axis === "X"
...
function removeExistingTimeline(parent) {
    const existing = [];
...
function svgIcon(svgString, name) {
...
function createTriggerIcon(triggerType) {
    switch (triggerType) {
...
function createTriggerPanel(triggerAction, triggerType, panelHeight) {
    ...
    const descText = createLabel("The animation of Step 1\nbegins when:", 16, TICK_TEXT_COLOR, "Regular");
...
function createDelayFallback(delayMs, width) {
...
// GERAÇÃO DA TIMELINE
// ============================================================
async function generateTimeline(targetNodeId, tracks, msPerTick = DEFAULT_MS_PER_TICK, triggerLabel = "", triggerAction = "", triggerType = "tap", triggerComponentName = "", stepTitle = "") {
    ...
    try {
        ...
        throw new Error(`Falha ao carregar fontes Inter: ${(fontErr === null || fontErr === void 0 ? void 0 : fontErr.message) || fontErr}`);
    }
    ...
    let nameComp;
    let delayComp = null;
    let timeContainerComp = null;
    let triggerFlowComp = null;
    const timelineComps = {};
    try {
        ...
        figma.ui.postMessage({
            ...
            success: false,
            error: `Erro ao importar Name-component da biblioteca "Library HO components". Verifique se a biblioteca está habilitada neste arquivo. (${(err === null || err === void 0 ? void 0 : err.message) || err})`,
        });
        return;
    }
    // Importar apenas as variantes necessárias do Timeline-container
    const neededVariants = new Set();
    ...
    for (const variant of neededVariants) {
        ...
        try {
            ...
            figma.ui.postMessage({
                type: "generate-result",
                success: false,
                error: `Erro ao importar Timeline-container variante "${variant}" da biblioteca. (${(err === null || err === void 0 ? void 0 : err.message) || err})`,
            });
            return;
        }
    }
    ...
    const hadExisting = removeExistingTimeline(targetNode);
    if (hadExisting) {
    ...
    const totalHeight = CONTAINER_PADDING + stepTitleOffset + RULER_HEIGHT + 24 + tracksHeight + 24 + RULER_HEIGHT + CONTAINER_PADDING;
    ...
    if (hasTrigger) {
        if (triggerFlowComp) {
            ...
            try {
                ...
                console.warn(`[Motion Timeline] Falha ao definir props do Trigger-Flow: ${propErr === null || propErr === void 0 ? void 0 : propErr.message}`);
            }
    ...
    if (timeContainerComp) {
        ...
        if (msPerTick !== 25) {
            try {
                const textNodes = timeContainerInst.findAll((n) => n.type === "TEXT" && n.name === "{Milisecond}");
                ...
                console.warn(`[Motion Timeline] Falha ao atualizar labels dos ticks: ${tickErr === null || tickErr === void 0 ? void 0 : tickErr.message}`);
            }
        }
    }
    else {
    ...
    cardsFrame.resize(totalWidth - cardsLeftX - CONTAINER_PADDING + CARDS_PADDING, cardsContainerHeight // Mesma altura que o Trigger Panel
    );
    ...
    for (let ti = 0; ti < tracks.length; ti++) {
        ...
        try {
            ...
            console.warn(`[Motion Timeline] Falha ao definir props de Name-component: ${propErr === null || propErr === void 0 ? void 0 : propErr.message}`);
        }
        // Auto-contrast: texto branco em backgrounds escuros, preto em claros
        const contrastColor = getContrastColor(trackColor);
        const letraNode = nameInstance.findOne((n) => n.name === "Letra" && n.type === "TEXT");
        if (letraNode) {
            letraNode.fills = [{ type: "SOLID", color: contrastColor }];
        }
        // Também ajustar ícone de link (quando visível)
        const linkVector = nameInstance.findOne((n) => n.name === "Vector" && n.type === "VECTOR");
        if (linkVector) {
            ...
            for (const prop of propsToRender) {
                ...
                if (delayWidthPx > 0) {
                    ...
                    if (delayComp) {
                        ...
                        try {
                            ...
                            console.warn(`[Motion Timeline] Falha ao definir delay prop: ${propErr === null || propErr === void 0 ? void 0 : propErr.message}`);
                        }
                ...
                if (comp) {
                    ...
                    try {
                        ...
                        console.warn(`[Motion Timeline] Falha ao definir props de Timeline-container (${variant}): ${propErr === null || propErr === void 0 ? void 0 : propErr.message}. Props tentadas: ${JSON.stringify(Object.keys(instanceProps))}`);
                    }
                    // Set easing on nested Token/Curves instance
                    const easingVariantValue = easingToVariant(anim.easing);
                    const tokenInst = timelineInstance.findOne((n) => n.name === "Token/Curves/Timeline-container" && n.type === "INSTANCE");
                    if (tokenInst) {
                        try {
                            tokenInst.setProperties({ "Curves token": easingVariantValue });
                        }
                        catch (propErr) {
                            console.warn(`[Motion Timeline] Falha ao definir easing: ${propErr === null || propErr === void 0 ? void 0 : propErr.message}`);
                        }
                    }
                    ...
                    const colorTargets = timelineInstance.findAll((n) => (n.name === "Text properties" || n.name === "Text time") && n.type === "FRAME");
                    for (const target of colorTargets) {
                        target.fills = [{ type: "SOLID", color: trackColor }];
                    }
                    // 2. Linha "Right" (vector) → stroke = trackColor
                    const lineVector = timelineInstance.findOne((n) => n.name === "Right" && n.type === "VECTOR");
                    if (lineVector) {
                    ...
                    const descTexts = timelineInstance.findAll((n) => n.type === "TEXT" && (n.name === "Description" || n.name === "Duration Text"));
    ...
    const totalText = createLabel(`Total: ${actualMaxMs}ms · ${totalAnims} anim · ${tracks.length} tracks`, 11, WHITE, "Medium");
    ...
    // =================== NOTIFICAR SUCESSO ===================
    figma.notify(`✅ Timeline gerada: ${tracks.length} track(s), ${totalAnims} animação(ões), ${actualMaxMs}ms total`, { timeout: 4000 });
    figma.ui.postMessage({
        type: "generate-result",
        success: true,
        data: {
        ...
        case "get-selection": {
            ...
            if (selection.length === 0) {
                figma.ui.postMessage({
                    ...
                    success: false,
                    error: "Nenhum elemento selecionado. Selecione um Frame ou Section no canvas.",
                });
        ...
        case "generate-timeline": {
            ...
            if (!hasAnims) {
                figma.ui.postMessage({
                    type: "generate-result",
                    success: false,
                    error: "Nenhuma animação definida. Adicione animações às suas tracks.",
                });
            ...
            generateTimeline(genMsg.targetNodeId, genMsg.tracks, msPerTick, trigLabel, trigAction, trigType, trigCompName, stepTitleVal).catch((err) => {
                var _a;
                const errorDetail = (err === null || err === void 0 ? void 0 : err.message) || ((_a = err === null || err === void 0 ? void 0 : err.toString) === null || _a === void 0 ? void 0 : _a.call(err)) || String(err);
                console.error("[Motion Timeline] Erro detalhado:", errorDetail, (err === null || err === void 0 ? void 0 : err.stack) || "");
                figma.ui.postMessage({
                    type: "generate-result",
                    success: false,
